...
Fabric mod for Minecraft 1.21.9-1.21.11. OpenAI groups inventory items client-side; server validates inventory state and performs every item mutation deterministically.
...
- `Alt+J`: use original deterministic sorter without OpenAI
- Full inventory + carried-shulker snapshot with SHA-256 stale-state rejection
- Structured Outputs through OpenAI Responses API
...
- `OPENAI_API_KEY`, session-only paste, or opt-in local persistence
...
Install mod and dependencies on client and server. Dedicated server needs no OpenAI key and makes no OpenAI requests.
...
Remembered key lives in `.minecraft/config/aishulkersorter.toml` as plaintext. Default is off. Keys are never logged, bundled, packetized, or sent to Minecraft server.
